module BABYLON {
    ...
    export class AnimationState extends BABYLON.ScriptComponent {
        ...
        private static DEG90:number = 90;
        ...
        public getDeltaPosition():BABYLON.Vector3 { return this._deltaPositionFixed; }
        public getDeltaRotation():BABYLON.Quaternion { return this._deltaRotation; }
        public getAngularVelocity():BABYLON.Vector3 { return this._angularVelocity; }
        ...
        protected m_avatarMask:Map<string, number> = null;
        ...
        private awakeStateMachine():void {
            ...
            this.m_avatarMask = new Map<string, number>();
                            ...
                            if (primaryBlendTree != null) {
                                if (layerState.blendtree.blendType == BABYLON.BlendTreeType.Clip) {
                                    ...
                                    if (animationTrack != null) {
                                        ...
                                        let correctionoffsety:number = 0.0;
                                        let orientationoffsety:number = 0.0;
                                        let keeporiginalorientation:boolean = false;
                                        let keeporiginalpositionxz:boolean = false;
                                        let keeporiginalpositiony:boolean = false;
                                        let loopblendorientation:boolean = false;
                                        let loopblendpositiony:boolean = false;
                                        let loopblendpositionxz:boolean = false;
                                        //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        private updateBlendableTargets(deltaTime:number, layer:BABYLON.IAnimationLayer, tree:BABYLON.IBlendTree, masterAnimation:BABYLON.TargetedAnimation, targetIndex:number, targetMixer:BABYLON.AnimationMixer, normalizedFrameTime:number, targetRootBone:boolean):void {
        ...
        private checkStateMachine(layer:BABYLON.IAnimationLayer, deltaTime:number):void {
            ...
            if (this._checkers.result != null) {
                if (this._checkers.offest > 0) BABYLON.SceneManager.SetTimeout((this._checkers.offest * 1000), ()=>{ this.setCurrentAnimationState(layer, this._checkers.result, this._checkers.blending); });
                else this.setCurrentAnimationState(layer, this._checkers.result, this._checkers.blending);
            }
        }
        private checkStateTransitions(layer:BABYLON.IAnimationLayer, transitions:BABYLON.ITransition[], time:number, length:number, rate:number):any {
            if (transitions != null && transitions.length > 0) {
                ...
                for(i = 0; i < transitions.length; i++ ) {
                    ...
                    // ..
                    const exitTimeSecs:number = BABYLON.Scalar.Denormalize(transition.exitTime, 0, length);
                    const exitTimeExpired:boolean = ((BABYLON.SceneManager.GetGameTime() - time) >= exitTimeSecs);
                    ...
                    if (transitionOk === true) {
                        const blendSize:number = (length > 0) ? length : 1;
                        const blendRate:number = (rate > 0) ? rate : BABYLON.AnimationState.FPS;
                        const destState:string = (transition.isExit === false) ? transition.destination : BABYLON.AnimationState.EXIT;
                        const offsetSecs:number = (transition.fixedDuration === true) ? transition.offset : BABYLON.Scalar.Denormalize(transition.offset, 0, blendSize)
                        const durationSecs:number = (transition.fixedDuration === true) ? transition.duration : BABYLON.Scalar.Denormalize(transition.duration, 0, blendSize);
        ...
        private checkAvatarTransformPath(layer:BABYLON.IAnimationLayer, transformPath:string):boolean {
            ...
            if (this.m_avatarMask != null) {
                const transformIndex:number = this.m_avatarMask.get(transformPath);
